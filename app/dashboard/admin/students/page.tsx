            ...
            <div className="flex gap-2 flex-wrap">
              {classes.slice(0, 7).map(c => (
                <button key={c} onClick={() => setFilterClass(c)}
                  ...
                  style={{ background: filterClass === c ? '#C9A84C' : '#1e2d47', color: filterClass === c ? '#0A0F1C' : '#9EA8B8' }}>
                  {c}
                </button>
              ))}
            </div>
                        ...
                        <td>
                          <div className="flex items-center gap-2">
                            <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold"
                              style={{ background: s.is_active ? '#C9A84C' : '#1e2d47', color: s.is_active ? '#0A0F1C' : '#5A6478' }}>
                              {s.full_name[0]}
                            </div>
                            <div>
                        ...
                        <td>
                          <div className="flex gap-2">
                            <button onClick={() => { setEditingId(s.id); setEditForm(s) }}
                              className="text-xs px-2 py-1 rounded" style={{ background: '#1e2d47', color: '#9EA8B8' }}>
                              Edit
                            </button>
