    ...
    <div className="min-h-screen bg-[#0A0F1C]">
      ...
      <main className="pt-[60px] pl-[248px] h-screen flex flex-col">
        <div className="flex-1 flex flex-col p-6 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto space-y-4 pb-4">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[75%] rounded-xl px-4 py-3 text-sm leading-relaxed ${
                  ...
                  style={{
                    background: msg.role === 'user' ? '#C9A84C' : '#141d2e',
                  ...
                  {msg.role === 'assistant' && (
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-5 h-5 rounded-full flex items-center justify-center text-xs"
                        style={{ background: '#C9A84C', color: '#0A0F1C' }}>AI</div>
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="rounded-xl px-4 py-3" style={{ background: '#141d2e', border: '1px solid #1e2d47' }}>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {suggestions.map(s => (
                <button key={s} onClick={() => setInput(s)}
                  ...
                  style={{ background: '#141d2e', border: '1px solid #1e2d47' }}>
                  {s}
                </button>
              ))}
            </div>
          ...
          {/* Input */}
          <div className="flex gap-3" style={{ borderTop: '1px solid #1e2d47', paddingTop: '1rem' }}>
            <input
              type="text"
              value={input}
